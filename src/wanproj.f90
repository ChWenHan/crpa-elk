...
! determine the projectors
  ...
  call putwanprjk('.OUT',ik,ld,nst,nproj,wanprj(:,:,:,:,:,ik))
end do
!$OMP END DO
!$OMP END PARALLEL
call freethd(nthd)
...
do ik=1,nkpt
  !write(*,*)'write ld',ld
  !write(*,*)'write ldwanprj',ldwanprj
  !call putwanprjk('.OUT',ik,ld,nst,nproj,wanprj(:,:,:,:,:,ik))
  !write(*,*)'after putwan'
  if (testiowanprj) then
    allocate(wanprj_(ld,nst,nspinor,norb,nproj,nkpt))
    fname=trim('WANPRJ_ELK')//trim('.OUT')
    call getwanprjk(fname,0,vkl(:,ik),ld,nst,nproj,wanprj_(:,:,:,:,:,ik))
    !write(*,*)'after getwan'
    if (any((wanprj(:,:,:,:,:,ik)-wanprj_(:,:,:,:,:,ik)).ne.0.d0)) then
      write(*,*)'wanprj IO not right for ik',ik
    else
      write(*,*)'wanprj IO checked for ik',ik
    end if
    deallocate(wanprj_)
  end if
end do
